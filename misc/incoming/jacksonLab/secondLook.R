...
if(!exists("igv"))
    igv <- start.igv("chr1:161185278-161186278", "hg38")
...
# 27361392
# 27365825
region.three <- function()
{
   tms <- TrenaMultiScore(tpad, "PTK2B")
   showGenomicRegion(igv, "PTK2B")
...
notSure <- function()
{
    ...
    i <- 2
    i <- 3
    showGenomicRegion(igv, with(tbl.hg38, sprintf("%s:%d-%d", chrom[i], start[i]-shoulder, end[i]+shoulder)))
